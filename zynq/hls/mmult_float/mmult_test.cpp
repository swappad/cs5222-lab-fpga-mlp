...
void matrix_multiply_ref(T offsets[CLASSES], T weights[CLASSES][FEAT],  T in[TILE_SIZE][FEAT], T out[TILE_SIZE][CLASSES])
{
...
int main(void)
{
	...
	T inputs[TILE_SIZE][FEAT];
	T matMult_sw[TILE_SIZE][CLASSES];
	T matMult_hw[TILE_SIZE][CLASSES];
	...
	for(int t=0; t < BATCH; t+= TILE_SIZE) {
